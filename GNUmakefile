...
# -----------------------------------------------------------------------------
# Get the number of cores for threaded build based on RAM amount
# Logic:
# <= 2GB RAM -> 1 Thread
# <= 4GB RAM -> 2 Threads
# >  4GB RAM -> 3 Threads
...
ifndef NPROCS
    # Linux üzerinde 'free' ve 'awk' kullanarak belleği oku ve thread sayısını belirle
    ifeq ($(OS), Linux)
        NPROCS := $(shell free -m | awk '/^Mem:/{ if ($$2 <= 2048) print 1; else if ($$2 <= 4096) print 2; else print 3; }')
    endif
    
    # Eğer yukarıdakiler çalışmazsa varsayılan değer
    ifndef NPROCS
        NPROCS := 1
    endif
endif

# Kontrol amaçlı ekrana yazdıralım (Make çalıştığında görünür)
$(info >> Raspberry Pi Detected Memory Logic: Selected NPROCS = $(NPROCS))


# -----------------------------------------------------------------------------
# Hardware Detection and Flag Selection
# -----------------------------------------------------------------------------

# Model ismini sistemden okuyoruz (null karakterleri temizleyerek)
DETECTED_MODEL := $(shell cat /sys/firmware/devicetree/base/model 2>/dev/null | tr -d '\0')

# Varsayılan olarak RPi 4 ayarlarını kabul edelim
RPI_CFLAGS := -O3 -march=armv8-a+fp+simd+crc -mtune=cortex-a72

# Eğer model isminde "Raspberry Pi 5" geçiyorsa ayarları güncelle
ifneq (,$(findstring Raspberry Pi 5,$(DETECTED_MODEL)))
    RPI_CFLAGS := -O3 -march=armv8.2-a+fp+simd+crc -mtune=cortex-a76
    $(info >> Detected Hardware: Raspberry Pi 5 (Optimized Flags Set))
else
    $(info >> Detected Hardware: Raspberry Pi 4 (or generic) (Default Flags Set))
endif
...
CMAKE_CONFIG = cmake $(BUILD_CMAKE_ARGS) \
                     ...
                     -DCMAKE_C_FLAGS="$(RPI_CFLAGS)" \
                     -DCMAKE_CXX_FLAGS="$(RPI_CFLAGS)"
