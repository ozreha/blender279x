...
compile_Python() {
  ...
  PRINT ""
  $_python -m pip install zstandard

  PRINT ""
  $_python -m pip install numpy --no-binary :all:
...
print_info() {
  ...
  if [ "$WITH_EMBREE" = false ]; then
    _1="-D WITH_CYCLES_EMBREE=OFF"
    PRINT "  $_1"
    _buildargs="$_buildargs $_1"
  fi
  ...
  PRINT ""
  PRINT ""
  PRINT "  make -j$THREADS BUILD_CMAKE_ARGS=\"$_buildargs\"" > ./make_for_pi.sh
  sh ./make_for_pi.sh
  END_TIME=$(date +%s)
