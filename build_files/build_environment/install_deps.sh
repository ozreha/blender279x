...
compile_Python() {
  ...
  if [ ! -d $_inst ]; then
    ...
    ./configure --prefix=$_inst --libdir=$_inst/lib --enable-ipv6 \
        ...
        --with-computed-gotos --with-pymalloc --enable-optimizations --with-lto
...
compile_Embree() {
  ...
  if [ ! -d $_inst ]; then
    ...
    cflags="$RPI_CFLAGS -flax-vector-conversions"
    echo "Embree    $cflags"
    #read
...
install_DEB() {
  ...
  # Model ismini sistemden okuyoruz
  RPI_MODEL_NAME=$(cat /sys/firmware/devicetree/base/model | tr -d '\0')
  
  # Varsayılan (RPi 4) Flagleri
  RPI_ARCH_FLAGS="-march=armv8-a+fp+simd+crc -mtune=cortex-a72"
  
  # RPi 5 kontrolü
  case "$RPI_MODEL_NAME" in
      *"Raspberry Pi 5"*)
          echo ">> Hardware Detected: Raspberry Pi 5"
          RPI_ARCH_FLAGS="-march=armv8.2-a+fp+simd+crc -mtune=cortex-a76"
          ;;
      *)
          echo ">> Hardware Detected: Raspberry Pi 4 (or generic)"
          # Yukarıda zaten RPi 4 varsayılan olarak tanımlıydı
          ;;
  esac

  #################### CFLAGS & CXXFLAGS SETTINGS ########################
  echo " "
  WARNING "Global compiler flags are being set based on detected hardware."
  
  # Flagleri birleştirip sistem geneline yayıyoruz (export)
  # -fPIC ve -O3 her ikisinde de ortak olduğu için buraya ekledim.
  export CFLAGS="-fPIC -O3 $RPI_ARCH_FLAGS"
  export CXXFLAGS="-fPIC -O3 $RPI_ARCH_FLAGS"
  
  RPI_CFLAGS="$CFLAGS"
  RPI_CXXFLAGS="$CXXFLAGS"
  
  echo "Active CFLAGS: $CFLAGS"
  echo "Active CXXFLAGS: $CXXFLAGS"
  echo " "
