...
compile_OCIO() {
  ...
  if [ ! -d $_inst ]; then
    ...
    #else
      cflags="-fPIC -O3 -march=armv8-a+fp+simd+crc -mtune=cortex-a53"
    #fi
...
compile_Embree() {
  ...
  if [ ! -d $_inst ]; then
    ...
    cflags="-fPIC -O3 -march=armv8-a+fp+simd+crc -mtune=cortex-a53 -flax-vector-conversions"
...
install_DEB() {
  ...
  export CFLAGS="-fPIC -O3 -march=armv8-a+fp+simd+crc -mtune=cortex-a53"
  export CXXFLAGS="-fPIC -O3 -march=armv8-a+fp+simd+crc -mtune=cortex-a53"
  echo "CFLAGS=$CFLAGS"
...
print_info() {
  ...
  if [ "$FFMPEG_SKIP" = false ]; then
    ...
    if [ -d $INST/ffmpeg ]; then
     ...
     _1="-D WITH_PLAYER=ON -D WITH_BLENDER=OFF"
  PRINT "  $_1"
  _buildargs="$_buildargs $_1"
  ...
  #_buildargs="-D WITH_BLENDER=OFF -D WITH_PLAYER=ON"
  ...
  RUNFILE=$INFO_PATH/build_linux/bin/blenderplayer
